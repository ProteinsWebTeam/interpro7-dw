...
def insert_protein_residues2(uri: str, residues_file: str):
    logger.info("creating webfront_proteinresidue")

    con = MySQLdb.connect(**url2dict(uri), charset="utf8mb4")
    cur = con.cursor()
    cur.execute("DROP TABLE IF EXISTS webfront_proteinresidue")
    cur.execute(
        """
    ...
    with SimpleStore(residues_file) as proteins:
        for i, (protein_acc, entries) in enumerate(proteins):
            for entry_acc, entry in entries.items():
                for descr, locations in entry["descriptions"].items():
                    args.append((
                        protein_acc,
