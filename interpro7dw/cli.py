...
def gen_tasks(config: configparser.ConfigParser) -> list[Task]:
    ...
    tasks = [
        ...
        Task(fn=interpro.oracle.matches.export_features,
             ...
             requires=["export-proteins"],
             scheduler=dict(type=scheduler, queue=queue, cpu=8, mem=4096, hours=9)),
        Task(fn=interpro.oracle.matches.export_uniprot_matches,
             args=(ipr_pro_uri, df.proteins, df.protein2matches),
             kwargs=dict(processes=8, tempdir=temp_dir),
             name="export-matches",
             requires=["export-proteins"],
             scheduler=dict(type=scheduler, queue=queue, cpu=8, mem=10000, hours=20)),
        ...
        Task(fn=interpro.oracle.matches.export_uniparc_matches,
             ...
             scheduler=dict(type=scheduler, queue=queue, cpu=8, mem=16000, hours=30)),
    ...
    xrefs_tasks = [
        ...
        Task(fn=interpro.xrefs.taxa.export_xrefs,
             ...
             requires=["export-matches", "export-proteomes",
                       ...
                       "export-taxa"],
             scheduler=dict(type=scheduler, queue=queue, cpu=16, mem=24000, hours=18)),
    ]
    tasks += xrefs_tasks
    tasks += [
        Task(fn=wait,
    ...
    tasks += [
        ...
        Task(fn=interpro.oracle.lookup.build_matches_table,
             ...
             name="lookup-matches",
             requires=["lookup-md5"],
             scheduler=dict(type=scheduler, queue=queue, mem=4000, hours=60)),
        Task(fn=interpro.oracle.lookup.build_site_table,
             args=(ips_pro_uri,),
             name="lookup-sites",
             requires=["lookup-md5"],
             scheduler=dict(type=scheduler, queue=queue, mem=4000, hours=30)),
    ...
    mysql_tasks = [
        ...
        Task(fn=interpro.mysql.databases.populate_rel_notes,
             ...
             scheduler=dict(type=scheduler, queue=queue, mem=12000, hours=10),
    ...
    service_tasks = [
        ...
        Task(
            ...
            name="export-goa",
            scheduler=dict(type=scheduler, queue=queue, mem=8000, hours=10),
