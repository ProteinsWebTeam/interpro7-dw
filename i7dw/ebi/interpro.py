...
def export_prot_matches(uri, dst, chunk_size=1000000):
    ...
    cur.execute(
        """
        SELECT M.PROTEIN_AC, LOWER(M.METHOD_AC), LOWER(E2M.ENTRY_AC), M.POS_FROM, M.POS_TO
    ...
    proteins = {}
    cnt = 0
    for acc, method_ac, entry_ac, start, end in cur:
        if acc in proteins:
            p = proteins[acc]
        else:
            if len(proteins) == chunk_size:
...
def get_pfam_clans(cur):
    ...
    cur.execute(
        """
        SELECT LOWER(c.clan_acc), c.clan_id, c.clan_description, c.number_sequences, LOWER(l.pfamA_acc_1), LOWER(l.pfamA_acc_2), l.evalue
        FROM clan c
        INNER JOIN clan_membership m ON c.clan_acc = m.clan_acc
        INNER JOIN pfamA2pfamA_hhsearch l ON m.pfamA_acc = l.pfamA_acc_1
        """
    )
