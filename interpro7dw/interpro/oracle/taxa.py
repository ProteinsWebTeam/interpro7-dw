...
def export_taxa(url: str, file: str):
    ...
    for row in cur:
        taxon_id = row[0]
        """
        Viruses are not a superkingdom anymore, but they aren't a domain either. 
        For the time being, we force the domain rank.
        ...
        taxa[taxon_id] = {
            ...
            "sci_name": row[2],
            "full_name": row[3],
            "rank": "domain" if row[2] == "Viruses" else row[4],
            "children": set(),
            "lineage": [taxon_id]
        }
