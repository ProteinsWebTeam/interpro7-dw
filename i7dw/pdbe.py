...
def get_secondary_structures(uri: str) -> dict:
    ...
    cur.execute(
        """
         SELECT H.ENTRY_ID, H.STRUCT_ASYM_ID, H.ELEMENT_TYPE, 
          R1.UNP_SEQ_ID AS POS_FROM, R1.CHEM_COMP_ID AS RES_FROM, 
          R2.UNP_SEQ_ID AS POS_TO, R2.CHEM_COMP_ID AS RES_TO
        FROM PDBE.SS_HELIX@PDBE_LIVE H
        INNER JOIN SIFTS_ADMIN.SIFTS_XREF_RESIDUE@PDBE_LIVE R1
          ON (H.ENTRY_ID=R1.ENTRY_ID 
            AND H.STRUCT_ASYM_ID=R1.STRUCT_ASYM_ID 
            AND H.RESIDUE_BEG_ID=R1.ID 
            AND R1.CANONICAL_ACC=1 
            AND R1.OBSERVED='Y')
        INNER JOIN SIFTS_ADMIN.SIFTS_XREF_RESIDUE@PDBE_LIVE R2
          ON (H.ENTRY_ID=R2.ENTRY_ID 
            AND H.STRUCT_ASYM_ID=R2.STRUCT_ASYM_ID 
            AND H.RESIDUE_END_ID=R2.ID 
            AND R2.CANONICAL_ACC=1 
            AND R2.OBSERVED='Y')
        UNION ALL
        SELECT S.ENTRY_ID, S.STRUCT_ASYM_ID, S.ELEMENT_TYPE, 
          R1.UNP_SEQ_ID AS POS_FROM, R1.CHEM_COMP_ID AS RES_FROM, 
          R2.UNP_SEQ_ID AS POS_TO, R2.CHEM_COMP_ID AS RES_TO
        FROM PDBE.SS_STRAND@PDBE_LIVE S
        INNER JOIN SIFTS_ADMIN.SIFTS_XREF_RESIDUE@PDBE_LIVE R1
          ON (S.ENTRY_ID=R1.ENTRY_ID 
            AND S.STRUCT_ASYM_ID=R1.STRUCT_ASYM_ID 
            AND S.RESIDUE_BEG_ID=R1.ID 
            AND R1.CANONICAL_ACC=1 
            AND R1.OBSERVED='Y')
        INNER JOIN SIFTS_ADMIN.SIFTS_XREF_RESIDUE@PDBE_LIVE R2
          ON (S.ENTRY_ID=R2.ENTRY_ID 
            AND S.STRUCT_ASYM_ID=R2.STRUCT_ASYM_ID 
            AND S.RESIDUE_END_ID=R2.ID 
            AND R2.CANONICAL_ACC=1 
            AND R2.OBSERVED='Y')
        """
    )
