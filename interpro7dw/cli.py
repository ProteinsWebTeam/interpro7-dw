...
def gen_tasks(config: dict) -> list[Task]:
    ...
    xrefs_tasks = [
        ...
        Task(fn=interpro.xrefs.proteomes.export_xrefs,
             args=(df.proteins, df.protein2matches,
                   df.protein2proteome, df.structures, df.uniprot2pdb, df.pdbematches, df.proteomes,
                   df.proteome2xrefs),
             kwargs=dict(processes=16, tempdir=temp_dir),
        ...
        Task(fn=interpro.xrefs.taxa.export_xrefs,
             args=(df.proteins, df.protein2matches, df.protein2proteome,
                   df.structures, df.uniprot2pdb, df.pdbematches, df.taxa, df.taxon2xrefs),
             kwargs=dict(processes=16, tempdir=temp_dir),
             name="export-taxon2xrefs",
             requires=["export-matches", "export-proteomes", "export-structures",
                       "export-uniprot2pdb", "export-pdb-matches",
                       "export-taxa"],
    ...
    mysql_tasks = [
        ...
        Task(fn=interpro.mysql.proteins.index_proteins,
             ...
             scheduler=dict(type=scheduler, queue=queue, mem=100, hours=16)),
